(() => {
  ...
  const messages = {ko: {
    ...
    source:'출처', stanza:'{number}연', sijo:'제{number}수', passage:'{number}구간',
    ...
    wrongHint:'{hint}', answerResult:'정답 {correct} · 선택 {selected}',
    missingOne:'답을 먼저 선택하세요.', view:'〈보기〉', reference:'참고 시구',
    toSummary:'핵심 정리로', toGeneral:'일반 문제로', toExam:'수능형 연습으로', toAdvanced:'기출 대조 실전으로',
    ...
    hintFallback:'표현의 주체와 어미, 앞뒤 문맥을 다시 확인하세요.'
  }};
  ...
  lesson.stanzas.forEach((text,i)=>{const part=el('div',undefined,'poetry-stanza');part.id=`${lesson.id}-stanza-${i+1}`;part.append(el('div',t(['sijo','passage'].includes(lesson.stanzaLabel)?lesson.stanzaLabel:'stanza',{number:i+1}),'poetry-stanza-number'),el('div',text,'poetry-text'));poemDetails.append(part);});
  ...
  const comparison=el('details',undefined,'poetry-comparison');comparison.append(el('summary',t('comparison',lesson.comparison)),el('div',lesson.comparison.text,'poetry-text'),link(`${lesson.comparison.author} 〈${lesson.comparison.title}〉`,lesson.comparison.url));
  ...
  learn.append(stageNav,heading,comparison,progress,itemNav,question,reference,view,choices,feedback,actions,notice,storageNotice);grid.append(poem,learn);
  mount.replaceChildren(top,hero,grid);
  ...
  function goStage(k){if(!order.includes(k)||!unlocked(k))return;state.stage=k;save();notice.textContent='';render();(k==='summary'?heading:question).focus();}
  ...
  function render(){
    ...
    heading.textContent=t(state.stage);question.hidden=state.stage==='summary';comparison.hidden=state.stage!=='advanced';poemLabel.textContent=state.stage==='advanced'?t('primary',lesson):t('read');
    ...
    if(state.stage==='summary'){
      question.textContent='';const dl=el('dl',undefined,'poetry-summary');lesson.summary.forEach(([title,text])=>dl.append(el('dt',title),el('dd',text)));feedback.append(dl);feedback.hidden=false;
